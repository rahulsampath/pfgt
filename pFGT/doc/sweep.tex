...
In most practical applications, unlike the uniform case, not all boxes exist and predominantly, there are two kinds of non-empty box distributions:
%
\begin{enumerate}
 ...
 \item Surface distribution, in which, each source box on an average has $\bigO(K^2)$ target boxes in its interaction list. These arise when the sources reside on the surface of a manifold (eg., sphere) embedded in the volume. 
\end{enumerate}
%   
% 
...
Suppose there are $n_b^3$ regular boxes sitting in $[0, 1]^3$ and $|B|$ of them are non-empty. There are two problems with the sweeping algorithm in the general case. First, a naive implementation of the sweeping algorithm stores data in all the $n_b^3$ boxes. This could be detrimental both for the volume and the surface distributions as, potentially, $|B| <\!< n_b^3$. Second, its performance is worse than the direct scheme for surface distributions. For example, consider the case where the sources are distributed just on the sphere.

In this section, we propose two algorithms: one for eliminating the storage requirements for volume distributions and achieving nearly optimal complexity and the other for achieving optimal complexity for the surface distributions. We also present a computationally inexpensive method that computes the cost of both algorithms {\em apriori} and picks the optimal of the two for any given distribution of nonzero boxes. 
