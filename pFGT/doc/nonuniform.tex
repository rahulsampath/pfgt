...
\item[\textbf{W2L}] Only the boxes that share sources with atleast one leaf node belonging to $T_e$ are non-empty, that is,
 carry a plane-wave expansion. Based on the distribution of the non-empty boxes, we choose between the modified sweeping
 algoritms for surface or volume distributions discussed in Section \ref{sc:sweep}. 

\item[\textbf{D2L}] This step is the dual of the W2D step. Here, we compute the influence of $T_d$ on $T_e$. 
For each source, $y$, within some direct leaf, we find all FGT boxes, $D$, such that $\mathcal{I}[y]$ intersects $D$ and
