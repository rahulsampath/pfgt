...
%to the plane-wave expansion for the FGT box, $B$, that contains $\ell$ using (\ref{eqn:s2w}). For each 
%leaf $\ell \in T_e$ such that $|\ell| > h$, visit each FGT box $B$ that is contained within $\ell$ and
%form the plane-wave expansion for $B$ using the sources that are contained within $B$ as shown in (\ref{eqn:s2w}).
