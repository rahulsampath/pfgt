...
\begin{enumerate}    
  ...
  \item The local expansion $v_k$ of the box $B(i+1, j+1)$ can be written in terms of the
    ...
    \end{eqnarray}
    The offsets $\chi, \xi$ represent the offsets of neighbors or the corners with respect to $B(i+1, j+1)$ on the unit cube defined at that scale. $\mathcal{M}(\chi), \mathcal{M}(\xi)$ is the ordinal of the contributing neighbor or corner, which in turn determines whether the contribution is positive or negative. The ordinal is simply the manhattan distance from the top-right corner on the unit cube at that scale. Therefore the top-right corner is 0, the corners one step away are $1$, and so on.
