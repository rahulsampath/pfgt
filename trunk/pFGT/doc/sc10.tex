...
Our algorithms scale as $ \bigO \left(\frac{N}{n_p} \right)$ for uniform point distributions and as $ \bigO\left(\frac{N}{n_p}\log \frac{N}{n_p} + n_p \log n_p\right)$ for nonuniform distributions on $n_p$ CPUs. 
We use our parallel octree construction paradigm (Sundar et al. ACM/IEEE SC `07) to efficiently handle nonuniform disrtibutions. We incorporate a plane-wave representation of the kernel which permits diagonal translation. 
...
Our implementation is {\em kernel-independent} as it can handle other numerous ``Gaussian-type'' kernels even when an analytic expression is lacking. These algorithms form a new class of core computational machinery for solving parabolic PDEs on massively parallel architectures. 
\end{abstract}
