...
\beq n \sqrt{ \delta \ln\left( \frac{1}{\epsilon} \right) } < n_{th} \eeq 
...
{\tt
\begin{algorithmic}
\STATE{Input : $N$ sources $\{(y_j, f_j) | j = 1..N\}$, $M$ targets $\{x_j | j = 1..M \}$, bandwidth $\delta$ and precision $\epsilon$}
\STATE
 \FOR {$j = 1 \quad \text{to} \quad N$}
    \STATE
    \STATE Find $\mathcal{I}[y_j]$: set of all the targets that within a distance $r = \sqrt{- \ln 
           \epsilon}$ from $y_j$
	 \STATE 
    \FOR {$\text{each}\quad x \in \mathcal{I}[y_j]$}
        \STATE{\sc Add contribution of $y_j$ to Gauss transform at $x$}
