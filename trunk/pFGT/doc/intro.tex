...
\begin{itemize} 
%
\item We present a parallel algorithm for the fast gauss transform (FGT). 
...
\ref{sc:nonuniform}, we present a new octree-based FGT algorithm for non-uniform point distributions; we also discuss its parallel implementation. Finally, in 
Section \ref{sc:results}, we demonstrate the scalability of our algorithms for uniform as well as non-uniform point distributions. 
In the rest of the paper, we stick to the case where $G_\delta$ is a Gaussian. This is purely for ease of exposition, all the
 concepts are valid in the general case too. For simplicity, we also assume that the sources and targets coincide and that they 
 lie within an unit cube.